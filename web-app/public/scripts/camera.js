            ...
            setTimeout(function(){
                if(ajax.responseText === "1"){
                    ...
                    delayFunction(function(){
                        ...
                        addStatus("location", "locationcoordinates", "ff0000", "#" + n + "<br>" + ajax.responseText);
                    });
                }
            },0);
        };
        ajax.onerror = function(){
            setTimeout(function(){
                delayFunction(function(){
                    ...
                    addStatus("location", "locationcoordinates", "ff0000", "#" + n + "<br>" + ajax.Error, retryButton);
                });
...
function setRecordDuration(t){
    recordDuration.innerText = addZero(Math.floor(t / 3600)) + ":" + addZero(Math.floor(t / 60)) + ":" + addZero(t % 60);
}
var chunk_n = 0;
...
function ajax(method, url, onload, onerror, formdata, chunk){
    var ajax = new XMLHttpRequest();
    ajax.onload = function(){
        onload(this, chunk);
    };
...
try{
    ...
    function sendLiveChunkAjaxOnerror(ajax, chunk){
        setTimeout(function(){
    ...
    function sendLiveChunk(chunk, additionalinfo){
        var formData = new FormData();
        ...
        ajax("POST", "../?live=1", sendLiveChunkAjaxOnload, sendLiveChunkAjaxOnerror, formData, chunk);
        liveUploadedChunksValue();
    }
    function liveStream(){
        // liveButton.disabled = 1;
...
function sendEmergencyModeSignalOnerror(ajax){
    ...
    retryButton.classList.add("buttons");
    addStatus("emergency", "emergencymode", "ff0000", "#" + liveN_2 + "<br>" + ajax.Error, retryButton);
